...
class PatientUpdate(BaseModel):
    name : Optional[str] = Field(None, max_length=100, description="Full name of the patient", example="John Doe")
    city : Optional[str] = Field(None, description="City of residence", example="New York")
    age : Optional[int] = Field(None, ge=0, le=120, description="Age of the patient", example=30)
    gender : Optional[Literal['male', 'female', 'other']] = Field(None, description ="Gender of the patient", example="male")
    height : Optional[float] = Field(None, ge=0, description="Height in meters", example=1.75)
    weight : Optional[float] = Field(None, gt=0, description="Weight in kilograms", example=70)
...
def update_patient(patient_id: str, patient_update: PatientUpdate):
    ...
    # existing_patient -> pydantic object -> update bmi + verdict
